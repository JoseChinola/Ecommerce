    ...
    return (
        <section className="bg-blue-50 rounded-md">
            <div className="container w-full mx-auto p-4 flex flex-col lg:flex-row gap-5 justify-between">
                ...
                {/* Section Address */}
                <div className="w-full bg-[#aeddd2] p-4 rounded-xl border">
                            ...
                            <label htmlFor={"address" + index} key={index} className={`w-full h-full ${!address.status && "hidden"}`} >
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-2 justify-center">
                                        ...
                                        <p className={`${Number(selectAddress) === index ? 'font-semibold' : 'text-gray-500'} line-clamp-2 capitalize text-xs`}>{address.address_line}</p>
                                    </div>
                                    ...
                                    <div className="mt-2 flex items-center gap-2 border-t pt-1">
                                        ...
                                        <p className={`text-xs  font-bold ${Number(selectAddress) === index ? 'text-green-500' : 'text-gray-500'}`}>{address.city}, {address.state}</p>
                                    </div>
                                    <div className="mt-2 flex items-center gap-2 pt-2">
                                        ...
                                        <p className={`text-xs  font-bold ${Number(selectAddress) === index ? 'text-green-500' : 'text-gray-500'}`}>{address.country} - {address.pincode}</p>
                                    </div>
                                    <div className="mt-2 flex items-center gap-2">
                                        <FaPhoneAlt size={18} className={`${Number(selectAddress) === index ? 'text-primary-Green' : 'text-gray-500'}`} />
                                        <p className={`text-xs  font-bold ${Number(selectAddress) === index ? 'text-green-500' : 'text-gray-500'}`}>{address.mobile}</p>
                                    </div>
                                </div>
                            </label>
                        ))}
                ...
                {/* Summary Section */}
                <div className="bg-[#aeddd2] w-full max-w-xs h-full py-4 px-3 rounded-xl border">
