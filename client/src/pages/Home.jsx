...
const Home = () => {
  ...
  useEffect(() => {
    ...
    if (prevCategoryIdsRef.current === currentIds) return
    ...
    <section className='bg-white rounded-lg'>
      <div className='container mx-auto rounded my-4 px-2 py-2'>
        ...
        <div className='w-full h-full grid items-center border bg-white gap-2 p-2 rounded-md'>
          <p className='font-semibold lg:text-xl italic px-4 py-2'>
            Ofertas hasta <span className='text-white bg-red-500 rounded-full w-fit text-center px-2'>-5%</span>
          </p>
          <Carousel />
        </div>
        ...
        {/* Categorías */}
        <div className='mt-4'>
          <h2 className='font-extrabold uppercase italic px-4 text-blue-500 bg-blue-100 py-2 rounded-md'>
            Category
          </h2>
        ...
        {categoriesWithProductsList.length > 0 &&
          categoriesWithProductsList.map((cat, idx) => (
