...
const CategoryPage = () => {
    ...
    return (
        <section className='p-3 bg-white rounded-lg'>
            <div className='py-4 px-2 rounded-md font-semibold bg-blue-50 shadow-md flex items-center justify-between flex-col md:flex-row'>
                <h2 className='font-extrabold uppercase text-primary-Green'>Categorías</h2>
                <button onClick={() => setOpenUploadCategory(true)} className='text-sm border text-green-500 font-semibold border-primary-Green hover:bg-primary-Green hover:text-white rounded flex px-3 py-1 items-center justify-center gap-1'>
                    <IoIosAddCircleOutline size={22} /> Agregar Categoría
                </button>
            </div>

            <div className='p-2 mt-4 grid gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6'>
                {loading ? (
                    Array.from({ length: 10 }).map((_, index) => (
                        <div
                            key={index}
                            className="animate-pulse bg-white shadow rounded-lg p-4 h-[200px] w-full"
                        >
                    ...
                    Array.isArray(data) && data.length > 0 ? (
                        data.map((category) => (
                            <div key={category._id} className="w-full max-w-xs mx-auto flex group flex-col bg-blue-50 gap-2 items-center rounded-md border-2 shadow-md relative">
                                <img src={category.image} alt={category.name} className="w-full h-44 object-cover rounded-md border-2" />
